...
app.get("/sandstorm/:ss", function(req, res) {
	var sandstormserver= req.params.ss;
	var myIP ="";
	var myPort = 0;
	var conct = "";
	if(sandstormserver == "ss1")
	{
		myIP = "178.63.72.133";
		myPort = 27041;
		conct = "steam://connect/178.63.72.133:27040";
	}
	else if(sandstormserver == "ss2")
	{
		myIP = "178.63.72.133";
		myPort = 27049;
		conct = "steam://connect/178.63.72.133:27048";
	}
	else
	{
		return;
	...
	Gamedig.query({
		...
		res.render("serversandstorm", {"GameFound": "true", "hostname": temp_Data.name, "servertype": "Sandstorm", "ip": temp_Data.query.address, "port": temp_Data.raw.port, "map": temp_Data.map, "gamemode": temp_Data.raw.rules.GameMode_s, "passwordprotected": passwordProtected, "vacsecure": VACsecure, "totalplayers": temp_Data.raw.numplayers, "maxplayers": temp_Data.raw.rules.PlrM_i, "playerlist": jsonPlayerName, "connectto": conct});
	}).catch((error) => {
...
app.get("/sandstorm/ss1/player/inventory", function(req, res) {
	res.setHeader('content-type', 'text/plain');
	res.send("Error! Please input SteamID64\nExample: /sandstorm1/player/inventory/76561198010486399");
	
	console.log("[WEB] %s connected to %s", GetIPAddress(req), req.originalUrl);
});

app.get("/sandstorm/ss1/player/inventory/:id", function(req, res) {
...
app.get("/sadnstorm/ss1/player", function(req, res) {
	console.log("[WEB] %s connected to %s", GetIPAddress(req), req.originalUrl);
	
	res.setHeader('content-type', 'text/plain');
	res.send("Error! Please input SteamID64\nExample: /sandstorm1/player/76561198010486399");
});

app.get("/sandstorm/ss1/player/:id", function(req, res) {
	console.log("[WEB] %s connected to %s", GetIPAddress(req), req.originalUrl);
	...
	{
		res.send("Error! Please input your steam ID\nExample: /sandstorm2/player/76561198010486399");
		return;
	}
});
/*SS2*/

app.get("/sandstorm/ss2/player/inventory", function(req, res) {
	res.setHeader('content-type', 'text/plain');
	res.send("Error! Please input SteamID64\nExample: /sandstorm1/player/inventory/76561198010486399");
...
app.get("/sandstorm/ss2/player/inventory/:id", function(req, res) {
	var hostinforaw = req.params.id;
	res.setHeader('content-type', 'text/plain');
...
app.get("/sadnstorm/ss2/player", function(req, res) {
	console.log("[WEB] %s connected to %s", GetIPAddress(req), req.originalUrl);
...
app.get("/sandstorm/ss2/player/:id", function(req, res) {
	console.log("[WEB] %s connected to %s", GetIPAddress(req), req.originalUrl);
...
app.get("/donate", function(req, res) {
	console.log("[WEB] %s connected to %s", GetIPAddress(req), req.originalUrl);
	res.redirect("https://fearless-assassins.com/donate/make-donation/");
});
...
app.get("/discord", function(req, res) {
	console.log("[WEB] %s connected to %s", GetIPAddress(req), req.originalUrl);
	res.redirect("https://discord.clan-fa.com/");
});
...
//Ins server
app.get("/:ins", function(req, res,next) {
	var insserver= req.params.ins;
	var myIP="";
	var myPort= 0;
	var conct="";
	...
	if(insserver == "ins1")
	{
		myIP = "108.61.21.93";
		myPort= 27019;
		conct = "steam://connect/108.61.21.93:27048";
	}
	else if(insserver == "ins2")
	{
		myIP = "178.63.72.133";
		myPort = 27020;
		conct = "steam://connect/178.63.72.133:27020";
	}
	else if(insserver == "ins3")
	{
		myIP = "108.61.21.93";
		myPort = 27030;
		conct = "steam://connect/108.61.21.93:27048";
	}
	else if(insserver == "ins4")
	{
		myIP = "178.63.72.133";
		myPort = 27022;
		conct = "steam://connect/178.63.72.133:27022";
	}
	else
	{
		next();
	...
	Gamedig.query({
		...
		res.render("server", {"GameFound": "true", "hostname": temp_Data.name, "servertype": "Insurgency", "ip": temp_Data.query.address, "port": temp_Data.raw.port, "customtheater": strCustomTheater, "map": temp_Data.map, "passwordprotected": passwordProtected, "friendlyfire": friendlyFire, "battleye": battleye, "vacsecure": VACsecure, "totalplayers": temp_Data.raw.numplayers, "maxplayers": temp_Data.maxplayers, "playerlist": jsonPlayerName, "connectto": conct});
	}).catch((error) => {
